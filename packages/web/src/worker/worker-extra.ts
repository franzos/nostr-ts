...
export function calculateEventsRequestRange(
  ...
  const since = filters.since || Math.round((Date.now() - ONE_DAY) / 1000);
...
export function relayEventsRequestFromQuery(
  ...
  const intervalAdjusted = Math.max(interval, ONE_DAY * 3);
  ...
  return {
    ...
    filters: {
      ...filters,
      since: query.filters.until - intervalAdjusted,
      until: undefined,
    },
    options: {
